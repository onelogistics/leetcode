 ...
 * (i, 0) 和 (i, ai), 找出其中的两条线，使得它们与x轴构成的容器可以盛最多的水
 */
public class MaxArea {
    /**
     * 解法分析：
     * 本题是求最大面积，两条线之间的面积为(j-i)*Math.min(ai,aj)，即长和高的乘积为容器的面积，从题目中我们可以得知
     * 最大的长即为(0,length-1)，那么我们可以用两个指针分别指向数组首和尾，此时计算出一个面积，然后我们移动指针，继续寻找是否有更大的面积
     * 在指针移动过程中，底也就是长度在减小，所以我们要找更高的线，这也就是指针移动的方向。
     * 时间复杂度O(N)
     *
     * @param height
     * @return
